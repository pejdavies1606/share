PROGRAM sine_lines ! mcoi = moving circle optical illusion
  ...
  INTEGER :: delay, rev, num, flim, i, f, ps, j, spin, size_x, size_y
  ...
  dt = 2.5 ! controls number of data points, and speed of rotation
  rev = 1 ! keep at 1, gifs repeat anyway
  dr = 1.75 ! 1.75 ! dr > 1 for hole in centre
  sf = 0.25 ! 0.25! amplitude of sineline
  sfaxes = 2.0 ! 2.0 ! scale factor for canvas border
  ps = 1 ! point size, line thickness
  w = 3.0 ! 2.0 ! speed of oscillation
  viewx = 60.0 ! 60.0
  ...
  th%n = 48
  phi%n = 48
  p%n = 12 ! for th or phi dependence, see DO loop below
...
!https://surreylearn.surrey.ac.uk/d2l/le/content/118114/viewContent/681905/View?ou=118114
  ...
  DO j=0,th%n ! cycle through dots
   DO i=0,phi%n
      ...
      IF ( dots(i,j)%th == th%s .OR. dots(i,j)%th == th%f ) THEN
        dots(i,j)%p = p%s
      ELSE
        dots(i,j)%p = MOD(p%s + (i+j)/2*p%d, p%f - p%s )
        ! set phase shift
  ...
  WRITE(11,'(a)') 'reset'
  WRITE(11,'(2a,3(a,i0))') 'set term gif animate opt background ',TRIM(bgnd),' delay ', delay, ' size ',size_x,',',size_y
